...
QUAD_LOGGER=$(docker run -d --link "${VIRTUOSO_IMAGE}:virtuoso" -e "HTTP_SERVER_URL=http://localhost:$LOGS_SERVER_PORT" -e "RUN_INTERVAL=60" -e "INSERT_PROCEDURE=y" -e "CUR_USER=$UID" -v $PWD/logs:/datadir jauco/virtuoso-quad-log)
echo "Launched! The quad logger!"
echo ""
echo "Browse to http://localhost:${LOGS_SERVER_PORT} to get started"
